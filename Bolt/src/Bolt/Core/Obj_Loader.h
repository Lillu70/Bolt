...
namespace Bolt
{
	...
	private:
		...
		std::string Read_Faces_In_Tight_Loop(std::ifstream& file, std::vector<glm::vec3>& positions, std::vector<glm::vec2>& texture_coord, std::vector<glm::vec3>& normals, std::unordered_map<Bolt::Vertex, uint32_t>& unique_vertices, std::vector<uint32_t>& indices, std::vector<Bolt::Vertex>& vertices);
